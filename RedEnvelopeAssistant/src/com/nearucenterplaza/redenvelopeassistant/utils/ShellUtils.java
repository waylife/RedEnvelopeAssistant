...
public class ShellUtils {
	...
	/**
	 * check whether has root permission
	 * 注意，使用百度一键root之后成功的�?�?<br/>
	 * @return
	 */
	public static CommandResult checkRootPermissionWithResult() {

		// if get permission,echo root
		// else no msg
		return execCommand("echo root", true, true);
	}

	/** 判断是否具有ROOT权限 ,此方法对有些手机无效，比如小米系�?*/
	...
	public static CommandResult execCommand(String[] commands, boolean isRoot,
		...
		try {
			...
			try{
				if(process!=null)
					process.exitValue();
			}catch(IllegalThreadStateException excepiton){
				if(process!=null)
					process.destroy();
			}catch (Exception ex){
				if(process!=null)
